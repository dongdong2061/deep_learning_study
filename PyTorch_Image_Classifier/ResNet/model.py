...
#reidual实线模块 18层和34层的resnet残差模块
...
class Bottleneck(nn.Module):
    ...
    def forward(self,x):
        ...
        out = self.bn3(out)
        print(out.size())
        out += identity  #将两个tensor的每个值相加，并不改变size
        print(out.size())
        out = self.relu(out)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self,block,channel,block_num,stride=1):
        ...
        layers = []
        layers.append(block(self.in_channel,channel,dowmsample=dowmsample,stride=stride))
    ...
    def forward(self,x):
        ...
        if self.include_top: #控制是否启动全连接层
            x = self.avgpool(x)
            x = torch.flatten(x,1)
            x = self.fc(x)
